	 ...
	 case FRIC_3508:{
		 	switch (str->PID_Type)
      {
        case Clear_Away:{
         ...
         default:{
          memcpy(&(str->Speed_Loop.PID_Param),&PID_Speed_Param[FRIC_3508][KEY],length);
        }break;
			}
   }	 
	}
...
{
	str->Motor_Data.PID_Speed_target = 6000;
...
void Shoot_Ctrl_FRIC_L(Shoot_Motor_t *str)
{
	str->Motor_Data.PID_Speed_target = -7000;
	str->Motor_Data.PID_Speed = str->Motor_Data.CAN_GetData.Motor_Speed;
}
void Shoot_Ctrl_FRIC_R(Shoot_Motor_t *str)
{
...
void Shoot_Ctrl_DRIVER(Shoot_Motor_t *str)
{
	if(KEY_C == 1)
	{
		str->Motor_Data.PID_Speed_target = -1000;
	}
	else{
		str->Motor_Data.PID_Speed_target = 5000;
	}
...
void Shoot_CanOutPut(void)
{
	...
	CAN_cmd_shoot (pid_out[FRIC_R],	pid_out[FRIC_L]);
}
